...
struct SystemLogEntry {
    ...
    var date: NSDate {
        let sec = Int(time) ?? 0
        let nanosec = Int(timeNanoSec) ?? 0
        let timeInterval = NSTimeInterval(sec) + NSTimeInterval(nanosec) * 1.0e-9
...
//        let object = asl_open_path(<#T##path: UnsafePointer<Int8>##UnsafePointer<Int8>#>, <#T##opts: UInt32##UInt32#>)
//        asl_vlog(<#T##obj: asl_object_t##asl_object_t#>, <#T##msg: asl_object_t##asl_object_t#>, <#T##level: Int32##Int32#>, <#T##format: UnsafePointer<Int8>##UnsafePointer<Int8>#>, <#T##ap: CVaListPointer##CVaListPointer#>)
    }
}